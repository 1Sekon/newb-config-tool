...
func _ready():
  ...
  window_width = ProjectSettings.get_setting("display/window/size/viewport_width")#get_viewport().content_scale_size.x
  window_height = ProjectSettings.get_setting("display/window/size/viewport_height")#get_viewport().content_scale_size.y
  #
...
func _input(event):
  ...
  if event is InputEventScreenTouch:
    ...
    if event.pressed:
      ...
      for node in n:
        if node.get_global_rect().has_point(event.position) and not get_node("../../../CanvasLayer/MainUI/Content/Visibility").button_pressed:
