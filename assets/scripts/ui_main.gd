...
func tween(target: Object, property: String, final_value: Variant, duration: float, trans: Variant, easing: Variant):
...
func _ready() -> void:
  ...
  #config_main.get_node("Container/ConfigContainer").reparent(config_main.get_node("Container/Scroll"), false)
...
func _on_visibility_toggled(toggled_on: bool) -> void:
  var n : Array[Node] = [get_node(^"Content/Config"), get_node(^"Content/Menu"), get_node(^"Content/OpenConfig")]
  for node in n:
